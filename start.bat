...
REM ------------------------------------------------------------
REM Bootstrap: finds/repairs Python, installs Tesseract OCR and
REM Python dependencies as needed. Writes the resolved Python
REM interpreter path to .python_path.
REM ------------------------------------------------------------
...
set "PY="
if exist ".python_path" set /p PY=<.python_path
if not defined PY (
    echo ERROR: Bootstrap did not report a Python interpreter.
    pause
    exit /b 1
)
echo.
...
"%PY%" -c "from src.adb_utils import verify_adb_connection; r=verify_adb_connection(); print('ADB:', r.get('adb_path', 'NOT FOUND')); print('Devices:', len(r.get('devices', [])))"
if %errorlevel% neq 0 (
    echo.
